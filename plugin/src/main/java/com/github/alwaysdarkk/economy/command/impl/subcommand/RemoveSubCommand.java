...
public class RemoveSubCommand extends CustomCommand {

    private final EconomyUserCache userCache;
    private final EconomyRepository economyRepository;

    public RemoveSubCommand(EconomyUserCache userCache, EconomyRepository economyRepository) {
        super("remove", "economy.admin", false, "remover");
    ...
    protected void onCommand(CommandSender commandSender, String[] arguments) {
        if (arguments.length != 2) {
            commandSender.sendMessage("§cUtilize /money remove <jogador> <quantia>.");
            return;
        }
        ...
        commandSender.sendMessage(String.format(
                "§aYeah! Você removeu §f%s §ade coins de §f%s§a.", NumberFormatter.format(amount), target.getName()));
    }
}